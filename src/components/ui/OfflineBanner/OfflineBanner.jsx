...
const OfflineBanner = () => {
  ...
  const failedItems   = useSelector((s) =>   
     (s.offlineQueue?.items ?? []).filter((i) => i.status === 'failed')
);                             // ← ADD
...
{bannerMode === 'error' && failedItems.length > 0 && (
  <div style={{ width: '100%', marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
    {failedItems.map((item) => (
      ...
      >
        <span style={{ flex: 1, fontSize: 11, color: p.main, fontWeight: 600 }}>
          {item.label}
        </span>
        <span style={{ fontSize: 10, color: p.main, opacity: 0.75, flex: 2 }}>
          {item.error || 'Failed'}
        </span>
        <button
          style={{ fontSize: 10, padding: '2px 8px', borderRadius: 999,
                   border: `1.5px solid ${p.main}`, background: 'transparent',
                   color: p.main, cursor: 'pointer', whiteSpace: 'nowrap' }}
          onClick={() => dispatch(retryItem(item.id))}
        >
          Retry
        </button>
        <button
          ...
          onClick={() => dispatch(cancelItem(item.id))}
        >
          Cancel
        </button>
      </div>
