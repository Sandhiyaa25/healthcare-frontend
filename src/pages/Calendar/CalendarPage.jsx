...
const CalendarPage = () => {
  ...
  // ✅ AFTER — Step 1: add state (after line 81)
const [searchText, setSearchText] = useState('');
  ...
  useEffect(() => {
    ...
    fetchStaffApi({ role_id: '', per_page: 50 })
      .then((r) => {
 ...
 if (view !== 'week') return [];
  ...
  const base = dayjs(selectedDate || today);
  const startOfWeek = base.subtract(base.day(), 'day'); // Sunday start
  ...
  return Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      day: d.date(),
      isCurrentMonth: d.month() + 1 === current.month,
    };
  });
}, [view, selectedDate, current]);
...
const displayDays = view === 'week' ? weekDays : calendarDays;
  ...
  // Step 3: filter eventsByDate using searchText (replace lines 174–183)
const eventsByDate = useMemo(() => {
  const map = {};
  ...
  events.forEach((ev) => {
    ...
    if (!d) return;
    // if (q) {
    //   const matchPatient = (ev.patient_name || '').toLowerCase().includes(q);
    //   const matchDoctor  = (ev.doctor_name  || '').toLowerCase().includes(q);
    //   if (!matchPatient && !matchDoctor) return; // skip non-matching
    // }
// 🔥 STATUS FILTER (ADD THIS FIRST)
if (filters.status && ev.status !== filters.status) return;

// 🔍 SEARCH FILTER
if (q) {
  const matchPatient = (ev.patient_name || '').toLowerCase().includes(q);
  const matchDoctor  = (ev.doctor_name  || '').toLowerCase().includes(q);
  if (!matchPatient && !matchDoctor) return;
}
  ...
  return map;
// }, [events, searchText]);   // ← add searchText as dependency
}, [events, searchText, filters.status]);

const totalFilteredEvents = useMemo(
  () => Object.values(eventsByDate).reduce((sum, arr) => sum + arr.length, 0),
  [eventsByDate]
);
...
<FilterGroup>
  <input
    ...
    style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #ccc', fontSize: 13 }}
  />
</FilterGroup>
...
<MonthGrid
  style={{
    gridTemplateColumns: view === 'week' ? 'repeat(7, 1fr)' : undefined,
  }}
>
