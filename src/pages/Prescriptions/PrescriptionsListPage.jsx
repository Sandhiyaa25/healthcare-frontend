...
import { setCurrentPage } from '../../store/prescriptions/prescriptionsSlice';
...
const PrescriptionsListPage = () => {
  ...
  const [verifyItem,   setVerifyItem]   = useState(null);
    ...
    const page     = useSelector((s) => s.prescriptions.currentPage);
  const dispatch = useDispatch();

  const load = useCallback((p = page, status = statusFilter) => {
    fetchList({ page: p, perPage: 5, status: status || undefined });
  ...
  //   setStatusFilter(val);
  //   setPage(1);
  //   fetchList({ page: 1, perPage: 5, status: val || undefined });
  // };
const handleStatusFilter = (e) => {
  const val = e.target.value;
  setStatusFilter(val);

  dispatch(setCurrentPage(1));  // ✅ FIX

  fetchList({ page: 1, perPage: 5, status: val || undefined });
};
  // const handlePageChange = (p) => {
  //   setPage(p);
  //   fetchList({ page: p, perPage: 5, status: statusFilter || undefined });
  // };
    const handlePageChange = (p) => {
    dispatch(setCurrentPage(p));     // → prescriptions/setCurrentPage in DevTools
    fetchList({ page: p, perPage: 5, status: statusFilter || undefined });
  };
  ...
  const columns = [
    ...
    // Change the Verified By column render:
{
  title: 'Verified By',
  key: 'verified_by',
  render: (_, row) => (
    <CellSub>
      {row.pharmacist_name || (row.verified_by ? `Pharmacist #${row.verified_by}` : '—')}
    </CellSub>
  ),
},
