...
export const queueAdd = async (action) => {
  ...
  label:        action.label,
  payload_enc:  encryptPayload(action.payload),
  endpoint_enc: encryptPayload({ endpoint: action.endpoint, method: action.method }),
  retries:      0,
  status:       'pending',   // ← NEW: 'pending' | 'failed'
  error:        null,        // ← NEW: stores failure reason from API
};
...
const items = (req.result || []).map((item) => ({
  ...
  error:      item.error  || null,        // ← NEW
  payload:    decryptPayload(item.payload_enc),
  ...decryptPayload(item.endpoint_enc),
}));
...
export const queueMarkFailed = async (id, errorMessage) => {
  try {
    const db    = await openQueueDB();
    const tx    = db.transaction(STORE_NAME, 'readwrite');
    const store = tx.objectStore(STORE_NAME);
    ...
    req.onsuccess = () => {
      if (req.result) {
        ...
        req.result.error   = errorMessage || 'Unknown error';
...
// ─── Reset failed item back to pending (for manual retry) ────────────────────
export const queueResetItem = async (id) => {
  try {
    const db    = await openQueueDB();
    const tx    = db.transaction(STORE_NAME, 'readwrite');
    const store = tx.objectStore(STORE_NAME);
    ...
    req.onsuccess = () => {
      if (req.result) {
        ...
        req.result.error   = null;
        req.result.retries = 0;
        store.put(req.result);
      }
    };
    return new Promise((res) => {
