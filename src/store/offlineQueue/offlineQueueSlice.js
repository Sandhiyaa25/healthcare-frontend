...
const offlineQueueSlice = createSlice({
  ...
  reducers: {
    ...
    // Transient failure — still retryable
syncItemFailed: (state, { payload: id }) => {
  if (!state.failedIds.includes(id)) state.failedIds.push(id);
},

// Permanent failure — carries error message, shown in UI
syncItemError: (state, { payload: { id, error } }) => {
  if (!state.failedIds.includes(id)) state.failedIds.push(id);
  // Update the item in-place so UI shows the error message
...
// User clicked "Retry" on a specific failed item
retryItem: (state, { payload: id }) => {
  state.failedIds = state.failedIds.filter((fid) => fid !== id);
  const found = state.items.find((i) => i.id === id);
  if (found) { found.status = 'pending'; found.error = null; }
},

// User clicked "Cancel" — remove from Redux (IDB removal handled in saga)
cancelItem: (state, { payload: id }) => {
  state.items        = state.items.filter((i) => i.id !== id);
  state.pendingCount = state.items.filter((i) => i.status !== 'failed').length;
  state.failedIds    = state.failedIds.filter((fid) => fid !== id);
},
...
export const {
  ...
  queueAction,
  setPendingCount, setQueueItems,
  syncStart, syncItemSuccess, syncItemFailed, syncItemError,
  retryItem, cancelItem,
  syncComplete,
  showOfflineBanner, hideBanner,
  drainQueue,
  loadQueueRequest, loadQueueSuccess,
